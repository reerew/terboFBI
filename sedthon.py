...
@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.اصلخ"))
async def a(event):
...
@sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.م (.*)"))
async def spammer(event):
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('اهلا مطوري صقر @SaQiier')
